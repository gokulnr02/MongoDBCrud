...
exports.CountrySave = async(request ,response)=>{
    try{
       ...
       const ID =(await Country.find().sort({ _id: -1 }).limit(1))[0].CountryID+1
        const CountryID={
            CountryID: (await Country.find({})).length ?(await Country.find().sort({ _id: -1 }).limit(1))[0].CountryID+1: (await Country.find({})).length+1
        }
